-- ========================================
-- 完整修复学校管理菜单配置并刷新超级管理员权限
-- 版本: 1.0.3
-- 日期: 2025-12-31
-- 说明: 修复菜单路径、图标格式，并重新分配超级管理员所有菜单权限
-- 使用方法: 在MySQL客户端或数据库管理工具中执行此脚本
-- ========================================

-- 步骤1: 修复学校管理菜单的图标格式（统一使用RemixIcon格式）
UPDATE sys_menu 
SET icon = 'ri:school-line',
    update_time = NOW()
WHERE id = 50 AND menu_name = '学校管理';

-- 步骤2: 修复学校管理子菜单的图标格式
UPDATE sys_menu 
SET icon = 'ri:building-line',
    update_time = NOW()
WHERE id = 51 AND menu_name = '校区管理';

UPDATE sys_menu 
SET icon = 'ri:briefcase-line',
    update_time = NOW()
WHERE id = 52 AND menu_name = '院系管理';

UPDATE sys_menu 
SET icon = 'ri:file-list-line',
    update_time = NOW()
WHERE id = 53 AND menu_name = '专业管理';

UPDATE sys_menu 
SET icon = 'ri:group-line',
    update_time = NOW()
WHERE id = 54 AND menu_name = '班级管理';

-- 步骤3: 确保学校管理子菜单的component路径正确（以/开头）
UPDATE sys_menu 
...
-- 步骤5: 删除超级管理员角色的所有现有菜单权限
DELETE FROM sys_role_menu WHERE role_id = 1;

-- 步骤6: 为超级管理员角色重新插入所有菜单权限
