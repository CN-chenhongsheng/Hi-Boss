...
SET component = '/system/school/campus/index',
    update_time = NOW()
WHERE id = 51 AND menu_name = '校区管理' AND (component IS NULL OR component NOT LIKE '/%');

UPDATE sys_menu 
SET component = '/system/school/department/index',
    update_time = NOW()
WHERE id = 52 AND menu_name = '院系管理' AND (component IS NULL OR component NOT LIKE '/%');

UPDATE sys_menu 
SET component = '/system/school/major/index',
    update_time = NOW()
WHERE id = 53 AND menu_name = '专业管理' AND (component IS NULL OR component NOT LIKE '/%');

UPDATE sys_menu 
SET component = '/system/school/class/index',
    update_time = NOW()
WHERE id = 54 AND menu_name = '班级管理' AND (component IS NULL OR component NOT LIKE '/%');

-- 步骤2: 确保学校管理菜单的path正确
UPDATE sys_menu 
SET path = '/system/school',
    update_time = NOW()
WHERE id = 50 AND menu_name = '学校管理' AND (path IS NULL OR path != '/system/school');
